...
function contentOriginal() {
  $url = Url::fromRoute('custom_form_add');
  //$add_link = ;
...
//  }
  ...
  function content() {
    ...
    foreach (BdContactStorage::getAll() as $id => $content) {
      // Row with attributes on the row and some of its cells.
      $editUrl = Url::fromRoute('custom_form_edit', array('id' => $id));
      $deleteUrl = Url::fromRoute('custom_form_delete', array('id' => $id));
